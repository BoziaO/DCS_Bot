...
// Statyczne metody
ticketStatsSchema.statics.generateDailyStats = async function(guildId, date = new Date()) {
  ...
  const stats = {
    ...
    ticketsResolved: closedTickets.filter(t => t.status === 'resolved').length,
  ...
  // Zapisz lub zaktualizuj statystyki
...
ticketStatsSchema.statics.getStatsForPeriod = async function(guildId, period, startDate, endDate) {
...
ticketStatsSchema.statics.getTopPerformers = async function(guildId, period = 'monthly', limit = 5) {
